...
class HappinessBar {
  ...
  applyPenalty(amount, successRate, canInstantZero = false) {
    ...
    // Allow happiness to hit 0 naturally
    // No special 1% protection anymore

    // If passing (successRate >= 0.80), minimum can now also be 0
...
class LevelSession {
  ...
  /**
   * Handle an order timing out before completion
   */
  onOrderTimedOut() {
    if (this.status !== "active") return;
